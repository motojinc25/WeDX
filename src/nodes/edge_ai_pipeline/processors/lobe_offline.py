...
from models.onnx_lobe.model import LobeClassification
...
class EdgeAINode(BaseNode):
    def __init__(self, settings, logger=logging.getLogger(__name__)):
        self.version = "0.1.0"
        self.name = "Lobe.ai ONNX (Offline)"
        ...
        self.configs = {}
        self.configs["models"] = {
            "Lobe.ai ONNX Classification": LobeClassification,
        }
        self.configs["instances"] = {}
        self.configs["status"] = {}
        self.configs["signature_filepaths"] = {}
        ...
        if self.settings["gui"]:
            ...
            with dpg.file_dialog(
                ...
                default_path=os.path.abspath(
                    os.path.join(current_path, "../../../models/onnx_lobe")
                ),
            ...
            with dpg.node(
                ...
                # Add signature file dialog buttons
                with dpg.node_attribute(attribute_type=int(Attribute.STATIC)):
                    with dpg.group(horizontal=True):
                        dpg.add_text(
                            "Select Signature : not yet",
                            bullet=True,
                            tag=dpg_node_tag + ":signaturefile:text",
                        )
                        dpg.add_button(
                            label="File",
                            ...
                            user_data=dpg_node_tag,
                            tag=dpg_node_tag + ":signaturefile",
                        )
                        dpg.add_button(
                            label="Netron",
                            show=False,
    ...
    def get_export_params(self, node_id):
        ...
        params["signature_filepath"] = ""
        if self.settings["gui"]:
            params["position"] = dpg.get_item_pos(dpg_node_tag)
        if dpg_node_tag in self.configs["signature_filepaths"]:
            params["signature_filepath"] = self.configs["signature_filepaths"][dpg_node_tag]
        return params
    ...
    def set_import_params(self, node_id, params):
        ...
        if "signature_filepath" in params and os.path.exists(params["signature_filepath"]):
            self.configs["signature_filepaths"][dpg_node_tag] = params["signature_filepath"]
            dpg.set_value(
                item=dpg_node_tag + ":signaturefile:text",
                value="Select Signature : selected",
            )
        if dpg_node_tag in self.configs["signature_filepaths"]:
            ...
            self.configs["instances"][dpg_node_tag].connect(
                signature_filepath=self.configs["signature_filepaths"][dpg_node_tag]
            )
            if self.configs["instances"][dpg_node_tag].is_connect():
                ...
                if self.settings["gui"]:
                    ...
                    dpg.disable_item(dpg_node_tag + ":signaturefile")
    ...
    def callback_button_connect(self, sender, data, user_data):
        ...
        if (
            ...
            dpg.enable_item(dpg_node_tag + ":signaturefile")
            dpg.hide_item(dpg_node_tag + ":modelfileNetron")

    def callback_file_dialog_model(self, sender, app_data, user_data):
        dpg_node_tag = user_data
        self.configs["signature_filepaths"][dpg_node_tag] = app_data["file_path_name"]
        dpg.set_value(
            item=dpg_node_tag + ":signaturefile:text", value="Select Signature : selected"
        )
    ...
    def callback_change_netron(self, sender, data, user_data):
        dpg_node_tag = user_data
        model_filepath = self.configs["instances"][dpg_node_tag].get_model_filepath()
        self.settings["netron"].reload(model_filepath)
        webbrowser.open(
